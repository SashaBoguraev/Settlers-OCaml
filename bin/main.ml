...
(* Turns list of ints into a string *)
...
(* Function that takes in a player and a resource and returns
   an updated player with that resource added to their deck. *)
...
(* Loops through the node list and returns a new player with all the correct
   additional resources from the first component of all their nodes *)
...
(* Loops through the node list and returns a new player with all the correct
   additional resources from the second component of all their nodes *)
...
(* Loops through the node list and returns a new player with all the correct
   additional resources from the third component of all their nodes *)
...
(* read-eval-print loop for a road placement
    Takes in a player, board, and boolean for setup since roads need
    to be played on setup. If user input is a -1, exit loop.
   if valid user input: place road, exit loop and print setup output
...
let rec repl_road (player : Game.Player.Player.t)
  ...
  let s = "Where would you like your road to be?" in
  print_endline s;
...
let rec repl_piece (player : Game.Player.Player.t) (playernum : int)
  ...
  print_int playernum;
  print_string ": ";
  let s = "Where would you like your settlement to be?  " in
  print_endline s;
  ...
  if input = -1 && not setup then (board, player, playernum)
    ...
    | _, (None, i) -> (
        match i with
        | 3 ->
            print_endline "not a possible location";
            repl_piece player playernum piece board setup
        | _ ->
            failwith
              "UNEXPECTED BEHAVIOR TURN BACK NOW YOUR LIFE IS IN GREAT DANGER")

(* read-eval-print loop for a city placement
   takes in a player and board and return an updated board and player
   if valid input: place city, exit loop and print setup output
   if invalid input: rerun loop*)
let rec repl_city (player : Game.Player.Player.t)
  ...
  let s =
    "Where would you like your city to be? \n\
     Type -1 to cancel. Possible locations are  "
  in
  print_endline s;
...
(* read-eval-print loop for trading
   Takes in a player and asks for user input on which resource they want
   traded out and traded for.
   if valid input: update player resources and return player
   if invalid input: rerun loop*)
let rec repl_trade (player : Game.Player.Player.t) : Game.Player.Player.t =
  let s =
    "What resourse would you like to trade in? Type 'cancel' to cancel "
  in
  print_endline s;
  let input = read_line () in
  match input with
  | s ->
      if String.lowercase_ascii s = "cancel" then player
      else if String.lowercase_ascii s = "wheat" then
        if Game.Player.Player.get_wheat player > 2 then (
          print_endline "What resource would you like: ";
          let input1 = read_line () in
          match input1 with
          | s1 ->
              let new_player = Game.Player.Player.trade_wheat player in
              if String.lowercase_ascii s1 = "wood" then
                Game.Player.Player.add_wood new_player
              else if String.lowercase_ascii s1 = "ore" then
                Game.Player.Player.add_ore new_player
              else if String.lowercase_ascii s1 = "sheep" then
                Game.Player.Player.add_sheep new_player
              else if String.lowercase_ascii s1 = "clay" then
                Game.Player.Player.add_clay new_player
              else if String.lowercase_ascii s1 = "wheat" then
                Game.Player.Player.add_wheat new_player
              else (
                print_endline "Invalid Input";
                repl_trade player))
        else (
          print_endline "Don't have enough of that resourse";
          repl_trade player)
      else if String.lowercase_ascii s = "ore" then
        if Game.Player.Player.get_ore player > 2 then (
          print_endline "What resource would you like: ";
          let input1 = read_line () in
          match input1 with
          | s1 ->
              let new_player = Game.Player.Player.trade_ore player in
              if String.lowercase_ascii s1 = "wood" then
                Game.Player.Player.add_wood new_player
              else if String.lowercase_ascii s1 = "ore" then
                Game.Player.Player.add_ore new_player
          ...
          print_endline "Don't have enough of that resourse";
          repl_trade player)
      else if String.lowercase_ascii s = "sheep" then
        if Game.Player.Player.get_sheep player > 2 then (
          print_endline "What resource would you like: ";
          let input1 = read_line () in
          match input1 with
          | s1 ->
              let new_player = Game.Player.Player.trade_sheep player in
              if String.lowercase_ascii s1 = "wood" then
                Game.Player.Player.add_wood new_player
              else if String.lowercase_ascii s1 = "ore" then
                Game.Player.Player.add_ore new_player
          ...
          print_endline "Don't have enough of that resourse";
          repl_trade player)
      else if String.lowercase_ascii s = "clay" then
        if Game.Player.Player.get_clay player > 2 then (
          print_endline "What resource would you like: ";
          let input1 = read_line () in
          match input1 with
          | s1 ->
              let new_player = Game.Player.Player.trade_clay player in
              if String.lowercase_ascii s1 = "wood" then
                Game.Player.Player.add_wood new_player
              else if String.lowercase_ascii s1 = "ore" then
                Game.Player.Player.add_ore new_player
          ...
          print_endline "Don't have enough of that resourse";
          repl_trade player)
      else if String.lowercase_ascii s = "wood" then
        if Game.Player.Player.get_wood player > 2 then (
          print_endline "What resource would you like: ";
          let input1 = read_line () in
          match input1 with
          | s1 ->
              let new_player = Game.Player.Player.trade_wood player in
              if String.lowercase_ascii s1 = "wood" then
                Game.Player.Player.add_wood new_player
              else if String.lowercase_ascii s1 = "ore" then
                Game.Player.Player.add_ore new_player
...
(* read-eval-print loop for building/trading on a turn
   Input: a player, board, and player_num for whos turn it is
   if valid user input: enter repl loop for desired action
   if invalid input: rerun loop*)
let rec build (player : Game.Player.Player.t) (board : Game.Board.SmallBoard.t)
  ...
  print_endline
    "Enter 1 to build a road, 2 to build a settlement, 3 to build a city, 4 to \
     trade, or 5 to end turn: ";
  let input =
    try int_of_string (read_line ()) with
    | Failure _ -> 0
    | _ -> int_of_string (read_line ())
  ...
  if input = 0 then (
    ...
    if
      Game.Player.Player.get_road_count player > 0
      && Game.Player.Player.get_clay player > 0
      && Game.Player.Player.get_wood player > 0
    then (
      ...
      print_endline "Not enough resources to do this";
      build player board player_num)
  else if input = 2 then
    if
      Game.Player.Player.get_settlement_count player > 0
      && Game.Player.Player.get_clay player > 0
      && Game.Player.Player.get_wood player > 0
      && Game.Player.Player.get_wheat player > 0
      && Game.Player.Player.get_sheep player > 0
    then (
    ...
    if
      Game.Player.Player.get_city_count player > 0
      && Game.Player.Player.get_wheat player > 1
      && Game.Player.Player.get_ore player > 2
    then (
    ...
    if
      Game.Player.Player.get_clay player > 2
      || Game.Player.Player.get_wood player > 2
      || Game.Player.Player.get_wheat player > 2
      || Game.Player.Player.get_sheep player > 2
      || Game.Player.Player.get_ore player > 2
    then
      let trade_player = repl_trade player in
      match trade_player with _ -> build trade_player board player_num
    else (
      print_endline "Not enough resources to do this";
      build player board player_num)
  else
...
(* Function that prints out the rules for the game. Takes in unit and returns unit *)
let print_rules (u : unit) : unit =
  ...
  print_endline "\n====TURNS====\n";
  print_endline
    "After setup, turn play will start with player 1, alternating turns after \
     that \n";
  print_endline
    "On each turn, a dice will be rolled and update player resources based on \
     the number of settlements a player has on a tile \n";
  print_endline
    "Each settlement on the tile that is rolled returns 1 of that resource to \
     a player, each city returns 2 \n";
  print_endline
    "The player who's turn it is then has the option to build, trade, or end \
     turn \n";
  print_endline
    "They can build either a settlement, road, or city if they have the \
     allocated resources, or trade 3 of the same resource for 1 of another \n";
...
(* takes in a tuple of players and a board and returns
   a triple of player player board that contains the
   updated players and board after a turn is executed. *)
let rec turn (count : int)
  ...
  print_endline (lst2str (Game.Player.Player.get_road_locations new_player_one));
  print_string "Player one has cities at: ";
  print_endline (lst2str (Game.Player.Player.get_city_locations new_player_one));
  ...
  print_string "Roads: ";
  print_endline
    (string_of_int (Game.Player.Player.get_road_count new_player_one));
  print_string "Cities: ";
  print_endline
    (string_of_int (Game.Player.Player.get_city_count new_player_one));
  print_string "Clay: ";
  ...
  print_endline (lst2str (Game.Player.Player.get_road_locations new_player_two));
  print_string "Player two has cities at: ";
  print_endline (lst2str (Game.Player.Player.get_city_locations new_player_two));
  ...
  print_string "Roads: ";
  print_endline
    (string_of_int (Game.Player.Player.get_road_count new_player_two));
  print_string "Cities: ";
  print_endline
    (string_of_int (Game.Player.Player.get_city_count new_player_two));
  print_string "Clay: ";
...
(* takes in two players, a board, runs turn function over and over
   Unitl a player reaches 5 victory points. Returns a string and player
   of the winning player. *)
let rec repl_turn turn_number player1 player2 board :
    string * Game.Player.Player.t =
  if
    Game.Player.Player.get_victory_points player1 >= 5
    && Game.Player.Player.get_victory_points player2 >= 5
  then ("There is a draw!", player2)
  else if Game.Player.Player.get_victory_points player1 >= 5 then
    ("The winner is Player 1", player1)
  else if Game.Player.Player.get_victory_points player2 >= 5 then
    ("The winner is Player 2", player2)
  else
...
let () =
  ...
  print_endline
    "Possible settlement locations are labeled 1-24 starting top left and \
     increase right then down.\n\
     Possible road locations are labeled 1-30 starting top left and increase \
     right then down.";
